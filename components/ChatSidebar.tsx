...
const ChatSidebar = ({ chats, chatId: propChatId }: Props) => {
  ...
  return (
    <div className="w-full h-screen p-4 bg-slate-950 flex flex-col border-r border-slate-800 shadow-2xl">
      {/* Header / Logo Area */}
      ...
      <div className="flex-1 overflow-y-auto mt-6 flex flex-col gap-2 pr-2 custom-scrollbar">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-2 mb-2">Your Chats</p>
          ...
          <Link
            ...
            <div
              className={cn(
                ...
                {
                  "bg-slate-800 text-white shadow-sm ring-1 ring-slate-700/50": chat.id === chatId,
                  "text-slate-400 hover:text-white hover:bg-slate-900": chat.id !== chatId,
                },
              )}>
              ...
              {chat.id === chatId && (
                <div className="absolute right-0 top-0 bottom-0 w-1 bg-blue-500 rounded-l shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
              )}
            </div>
          </Link>
        ))}
      ...
      <div className="mt-4 pt-4 border-t border-slate-800/60">
        <div className="flex flex-col gap-1">
          <Link
            href="/"
            className="flex items-center p-2 rounded-md text-slate-400 hover:text-white hover:bg-slate-900 transition-colors text-sm">
            <Home className="w-4 h-4 mr-3" />
            Home
          </Link>
          <p className="text-[10px] text-slate-600 px-2 mt-2">© 2025 PDF Chat AI</p>
        </div>
      </div>
    </div>
  );
